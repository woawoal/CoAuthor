...
async def _get_world_or_404(world_id: uuid.UUID, db: AsyncSession) -> World:
...
async def list_characters(world_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    await _get_world_or_404(world_id, db)
    result = await db.execute(select(Character).where(Character.world_id == world_id))
    return result.scalars().all()


@router.post("/", response_model=CharacterResponse, status_code=201)
async def create_character(world_id: uuid.UUID, body: CharacterCreate, db: AsyncSession = Depends(get_db)):
    await _get_world_or_404(world_id, db)
    character = Character(world_id=world_id, **body.model_dump())
    db.add(character)
    await db.flush()
    await db.refresh(character)
    logger.info("캐릭터 생성: %s (world=%s)", character.name, world_id)
    return character
