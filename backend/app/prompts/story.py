...
- speaker            : dialogue를 말하는 인물 이름.
                       ① 조연이 있는 장면: [AI 서술 인물] 중 하나. 대사 없으면 빈 문자열.
                       ② 주인공이 혼자인 장면: 주인공 이름도 가능 — 이때 dialogue에 주인공의 독백·내면을 씀.
- dialogue           : speaker가 이번 턴에 하는 대사. 사용자 대사와 같은 감탄·반응·주제를 의미적으로도 반복하지 않는다. 따옴표 붙이지 않음. 대사 없으면 빈 문자열.
- protagonist_dialogue: [지시가 있을 때만] 주인공이 자연스럽게 할 법한 대사 한 문장. 지시 없으면 빈 문자열.
- ★[장면 존재 규칙 — 최우선 엄수]
  speaker로 지정하는 인물은 **지금 이 장면에 실제로 함께 있는 인물**이어야 한다.
  나레이션에서 "혼자", "아무도 없", "텅 빈", "홀로" 등을 서술했다면:
  → [AI 서술 인물]의 speaker·dialogue는 반드시 빈 문자열.
  → 주인공 혼자인 장면에서만 speaker=주인공, dialogue=독백 허용.
