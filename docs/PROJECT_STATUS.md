<!-- markdownlint-disable MD022 MD032 MD031 MD036 MD060 -->
# NodeVelture 프로젝트 현황
...
**다음 할 일 (2주차)**
- [ ] **🔥 캐릭터별 RAG 정보 분리** — 강사님 1순위 차별점 (각 캐릭터가 자기만 아는 정보)
...
| 리스크 | 내용 | 대응 |
|--------|------|------|
| **RAG 부재** | 강사님 1순위 차별점 "캐릭터별 RAG 정보 분리"가 MongoDB 제거로 사라짐. 발표 차별점인데 구현 없음 | pgvector로 복구 검토 |
...
## 5. 발표 차별점 방어 논리 (강사님 강조 — 6/2 스크럼)

> "ChatGPT/제미나이에 채팅하는 것과 뭐가 다른가" → 발표 때 반드시 나올 질문

1. 멀티 페르소나 — 캐릭터마다 RAG로 다른 정보, 서로 모르는 상태에서 대화
2. 대화 → 소설 자동 변환
3. 동기부여 시스템 (다른 유저 현황 등)
4. 삽화 자동 생성
